...
if ( ! function_exists( 'krown_id_percentage_raised' ) ) {

	function krown_id_percentage_raised( $content ) {
		$content = str_replace( array( '.00', ',' ), '', $content );
		return $content;
	}
...
add_filter( 'id_price_format', 'krown_id_funds_raised', 100, 3 );
add_filter( 'id_percentage_raised', 'krown_id_percentage_raised', 100, 3 );

remove_action( 'id_content_after', 'mdid_backers_list' );

function krown_media_views_script() {
	wp_enqueue_style( 'the-media-views', includes_url() . 'css/media-views.css' );
}
add_action('wp_enqueue_scripts', 'krown_media_views_script');
...
function krown_id_ago($time) {
	
   $periods = array( __("second", "krown"), __("minute", "krown"), __("hour", "krown"), __("day", "krown"), __("week", "krown"), __("month", "krown"), __("year", "krown"), __("decade", "krown") );
